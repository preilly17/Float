...
export class AuthService {
  ...
  static async login(data: LoginData): Promise<User> {
    ...
    // Some legacy usernames may contain "@", so fall back to username lookup
    // if email lookup does not return a match.
    let user: User | undefined;

    if (normalizedIdentifier.includes('@')) {
      // Prefer email lookup for identifiers containing '@'
      ...
      // Prefer username lookup for standard usernames
      user = await storage.getUserByUsername(this.normalizeUsername(normalizedIdentifier));
      if (!user) {
        user = await storage.getUserByEmail(this.normalizeEmail(normalizedIdentifier));
      }
