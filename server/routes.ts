...
export function setupRoutes(app: Express) {
  ...
  app.get("/api/photos/search", async (req, res) => {
    try {
      ...
      const page = pageRaw && !isNaN(Number(pageRaw)) ? parseInt(pageRaw as string, 10) : 1;

      const result = await pexelsService.searchPhotos(query, page, 15);
  ...
  app.get("/api/photos/curated", async (req, res) => {
    try {
      ...
      const result = await pexelsService.getCuratedPhotos(page, 15);
      return res.json(result);
    } catch (error) {
      console.error("Curated photos error:", error);
      return res.status(500).json({ error: "Failed to fetch curated photos" });
    }
  });

  app.get("/api/photos/status", async (_req, res) => {
    return res.json({ configured: pexelsService.isConfigured() });
  });
  ...
  app.get("/api/trips", async (req: any, res) => {
    try {
      ...
      // Add planning data counts for each trip
      // Status explanations for this domain:
      // - Flights: "confirmed"/"booked" = actual booking made
      // - Hotels: "confirmed"/"booked" = actual booking, "scheduled" = added via Schedule & Invite mode (confirmed booking)
      // - Activities: "active" = confirmed activity in DB, "confirmed"/"scheduled" = alternative confirmed statuses
      const tripsWithPlanningData = await Promise.all(
        trips.map(async (trip) => {
          const [flights, hotels, activities, packingItems] = await Promise.all([
            storage.getTripFlights(trip.id),
            storage.getTripHotels(trip.id),
            storage.getTripActivities(trip.id, userId),
            storage.getTripPackingItems(trip.id),
          ]);
          
          // Return planning data at top level for consistency with TripWithPlanningData type
          return {
            ...trip,
            flights,
            hotels,
      ...
      res.json(tripsWithPlanningData);
    } catch (error: unknown) {
  ...
  app.post("/api/trips/:id/check-date-impact", async (req: any, res) => {
    try {
      let effectiveUserId = getRequestUserId(req);
      ...
      const isWithinRange = (eventDate: Date | string | null) => {
        if (!eventDate) return true;
        const date = new Date(eventDate);
        return date >= newStart && date <= newEnd;
      };

      const activities = await storage.getActivitiesByTripCalendar(tripId);
      const flights = await storage.getFlightsByTrip(tripId);
      const hotels = await storage.getHotelsByTrip(tripId);
      const restaurants = await storage.getRestaurantsByTrip(tripId);

      const affectedActivities = activities.filter(a => {
        const startTime = a.startTime ? new Date(a.startTime) : null;
        return startTime && !isWithinRange(startTime);
      });

      const affectedFlights = flights.filter(f => {
        const departure = f.departureTime ? new Date(f.departureTime) : null;
        return departure && !isWithinRange(departure);
      });

      const affectedHotels = hotels.filter(h => {
        const checkIn = h.checkInDate ? new Date(h.checkInDate) : null;
        const checkOut = h.checkOutDate ? new Date(h.checkOutDate) : null;
        return (checkIn && !isWithinRange(checkIn)) || (checkOut && !isWithinRange(checkOut));
      });

      const affectedRestaurants = restaurants.filter(r => {
        const reservationDate = r.reservationDate ? new Date(r.reservationDate) : null;
        return reservationDate && !isWithinRange(reservationDate);
      });

      res.json({
        hasAffectedEvents: affectedActivities.length > 0 || affectedFlights.length > 0 || 
                          affectedHotels.length > 0 || affectedRestaurants.length > 0,
        summary: {
          activities: affectedActivities.length,
          flights: affectedFlights.length,
          hotels: affectedHotels.length,
          restaurants: affectedRestaurants.length,
        },
        details: {
          activities: affectedActivities.map(a => ({ id: a.id, name: a.name, date: a.startTime })),
          flights: affectedFlights.map(f => ({ id: f.id, airline: f.airline, flightNumber: f.flightNumber, date: f.departureTime })),
          hotels: affectedHotels.map(h => ({ id: h.id, name: h.hotelName, checkIn: h.checkInDate, checkOut: h.checkOutDate })),
          restaurants: affectedRestaurants.map(r => ({ id: r.id, name: r.name, date: r.reservationDate })),
        }
      });
    } catch (error) {
      console.error("Error checking date impact:", error);
      res.status(500).json({ message: "Failed to check date impact" });
    }
  });
  ...
  app.post('/api/trips/:id/transfer-ownership', async (req: any, res) => {
    try {
      const tripId = parseInt(req.params.id);
      let userId = getRequestUserId(req);
      ...
      const updatedTrip = await storage.transferTripOwnership(tripId, userId, newOwnerId);
      ...
      const errorMessage = getErrorMessage(error);
      if (errorMessage.includes("Only the trip creator") || errorMessage.includes("must be a member")) {
        res.status(403).json({ message: errorMessage });
      } else if (errorMessage.includes("not found")) {
        res.status(404).json({ message: errorMessage });
      } else {
        res.status(500).json({ message: "Failed to transfer trip ownership" });
      }
    }
  });
  ...
  app.post('/api/trips/:id/restaurants', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const { selectedMemberIds, ...restaurantData } = rawBody;
      
      const validatedData = insertRestaurantSchema.parse({
        ...restaurantData,
        reservationDate: restaurantData.reservationDate ?? restaurantData.reservation_date,
  ...
  app.patch('/api/restaurants/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const rawBody = req.body ?? {};
      const reservationDateIso = rawBody.reservationDate 
        ? toDateOnlyIsoString(rawBody.reservationDate) 
        : undefined;
      ...
      const updated = await storage.updateRestaurant(restaurantId, {
        ...
        cuisineType: rawBody.cuisineType,
        priceRange: rawBody.priceRange,
        rating: rawBody.rating,
        phoneNumber: rawBody.phoneNumber,
        website: rawBody.website,
        openTableUrl: rawBody.openTableUrl,
        reservationDate: reservationDateIso,
        reservationTime: rawBody.reservationTime,
        partySize: rawBody.partySize,
        confirmationNumber: rawBody.confirmationNumber,
        reservationStatus: rawBody.reservationStatus,
        specialRequests: rawBody.specialRequests,
        notes: rawBody.notes,
      }, userId);
  ...
  app.get('/api/restaurants/:id/rsvps', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const rsvps = await storage.getRestaurantRsvps(restaurantId);
      res.json(rsvps);
    } catch (error) {
      console.error("Error fetching restaurant RSVPs:", error);
      res.status(500).json({ message: "Failed to fetch RSVPs" });
    }
  });

  app.post('/api/restaurants/:id/rsvp', isAuthenticated, async (req: any, res) => {
    try {
      const restaurantId = parseInt(req.params.id, 10);
      const userId = getRequestUserId(req);
      ...
      const rsvp = await storage.respondToRestaurantRsvp(restaurantId, userId, status);
      res.json(rsvp);
    } catch (error) {
      console.error("Error responding to restaurant RSVP:", error);
      res.status(500).json({ message: "Failed to respond to RSVP" });
    }
  });
  ...
  const handlePostTripHotelProposals = async (req: any, res: any) => {
    try {
      ...
      // Validate that check-in and check-out dates are provided for hotel proposals
      const hasHotelId = parsedData.hotelId != null || parsedData.id != null;
      if (!hasHotelId) {
        if (!parsedData.checkInDate) {
          return res.status(400).json({ message: "Check-in date is required for hotel proposals" });
        }
        if (!parsedData.checkOutDate) {
          return res.status(400).json({ message: "Check-out date is required for hotel proposals" });
        }
      }
      ...
      // Check if this is a pure proposal (status = 'proposed') - don't create an actual hotel
      const isProposalOnly = parsedData.status === 'proposed';

      if (!Number.isFinite(hotelId)) {
        // If status is 'proposed', skip hotel creation and create standalone proposal
        if (parsedHotelPayload && !isProposalOnly) {
          try {
          ...
          // Create a standalone proposal without a hotel record
          // Extract location parts for address/city/country defaults
          const locationStr = typeof parsedData.location === 'string' ? parsedData.location : '';
          const locationParts = locationStr.split(',').map((s: string) => s.trim()).filter(Boolean);
          ...
          const fallbackProposalInput = {
            ...
            location:
              typeof parsedData.location === 'string' && parsedData.location.trim().length > 0
                ...
                : 'Unknown Location',
            address:
              typeof parsedData.address === 'string' && parsedData.address.trim().length > 0
                ? parsedData.address
                : locationParts[0] || 'TBD',
            city:
              typeof parsedData.city === 'string' && parsedData.city.trim().length > 0
                ? parsedData.city
                : locationParts[0] || 'TBD',
            country:
              typeof parsedData.country === 'string' && parsedData.country.trim().length > 0
                ? parsedData.country
                : locationParts[1] || locationParts[0] || 'TBD',
            guestCount:
              parsedData.guestCount != null
                ? Number(parsedData.guestCount) || 1
                : 1,
            checkInDate: parsedData.checkInDate as string,
            checkOutDate: parsedData.checkOutDate as string,
            price:
  ...
  app.post('/api/hotel-proposals/:id/convert', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('only') || error.message.includes('permission')) {
          return res.status(403).json({ message: error.message });
        }
        if (error.message.includes('not found')) {
          return res.status(404).json({ message: error.message });
  ...
  app.post('/api/flight-proposals/:id/convert', isAuthenticated, async (req: any, res) => {
    try {
      const proposalId = Number.parseInt(req.params.id, 10);
      if (Number.isNaN(proposalId)) {
        return res.status(400).json({ message: "Invalid proposal id" });
      ...
      const proposal = await storage.confirmFlightProposal(proposalId, userId);
      res.json(proposal);

      broadcastToTrip(proposal.tripId, {
        type: "flight_proposal_confirmed",
        tripId: proposal.tripId,
        proposalId: proposal.id,
        triggeredBy: userId,
      });
    } catch (error: unknown) {
      console.error("Error converting flight proposal:", error);
      if (error instanceof Error) {
        if (error.message.includes('only') || error.message.includes('permission') || error.message.includes('Only')) {
          return res.status(403).json({ message: error.message });
        }
        if (error.message.includes('not found')) {
          return res.status(404).json({ message: error.message });
      ...
      res.status(500).json({ message: "Failed to convert flight proposal" });
    }
  });
  ...
  app.post('/api/restaurant-proposals/:id/convert', isAuthenticated, async (req: any, res) => {
    try {
      const proposalId = Number.parseInt(req.params.id, 10);
      if (Number.isNaN(proposalId)) {
        return res.status(400).json({ message: "Invalid proposal id" });
      ...
      const proposal = await storage.updateRestaurantProposalStatus(proposalId, "scheduled", userId);
      res.json(proposal);

      broadcastToTrip(proposal.tripId, {
        type: "restaurant_proposal_scheduled",
        tripId: proposal.tripId,
        proposalId: proposal.id,
        triggeredBy: userId,
      });
    } catch (error: unknown) {
      console.error("Error converting restaurant proposal:", error);
      if (error instanceof Error) {
        if (error.message.includes('only') || error.message.includes('permission')) {
          return res.status(403).json({ message: error.message });
        }
      ...
      res.status(500).json({ message: "Failed to convert restaurant proposal" });
    }
  });

  app.patch('/api/restaurant-proposals/:id', isAuthenticated, async (req: any, res) => {
    try {
      const proposalId = Number.parseInt(req.params.id, 10);
      if (Number.isNaN(proposalId)) {
        return res.status(400).json({ message: "Invalid proposal id" });
      ...
      if (proposal.proposedBy !== userId) {
        return res.status(403).json({ message: "You can only edit proposals you created" });
      }

      const updated = await storage.updateRestaurantProposal(proposalId, req.body, userId);
      res.json(updated);
    } catch (error: unknown) {
      console.error("Error updating restaurant proposal:", error);
  ...
  app.get('/api/flights/:id/rsvps', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const rsvps = await storage.getFlightRsvps(flightId);
      res.json(rsvps);
    } catch (error) {
      console.error("Error fetching flight RSVPs:", error);
      res.status(500).json({ message: "Failed to fetch RSVPs" });
    }
  });

  app.post('/api/flights/:id/rsvp', isAuthenticated, async (req: any, res) => {
    try {
      const flightId = parseInt(req.params.id, 10);
      const userId = getRequestUserId(req);
      const { status } = req.body;
      ...
      const rsvp = await storage.respondToFlightRsvp(flightId, userId, status);
      res.json(rsvp);
    } catch (error) {
      console.error("Error responding to flight RSVP:", error);
      res.status(500).json({ message: "Failed to respond to RSVP" });
    }
  });
  ...
  app.post('/api/trips/:id/hotels', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const { selectedMemberIds, ...hotelBody } = req.body;
      const normalizedHotelData = normalizeHotelRequestBody(hotelBody, tripId);
  ...
  app.get('/api/hotels/:id/rsvps', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const rsvps = await storage.getHotelRsvps(hotelId);
      res.json(rsvps);
    } catch (error) {
      console.error("Error fetching hotel RSVPs:", error);
      res.status(500).json({ message: "Failed to fetch RSVPs" });
    }
  });

  app.post('/api/hotels/:id/rsvp', isAuthenticated, async (req: any, res) => {
    try {
      const hotelId = parseInt(req.params.id, 10);
      const userId = getRequestUserId(req);
      const { status } = req.body;
      ...
      const rsvp = await storage.respondToHotelRsvp(hotelId, userId, status);
      res.json(rsvp);
    } catch (error) {
      console.error("Error responding to hotel RSVP:", error);
      res.status(500).json({ message: "Failed to respond to RSVP" });
    }
  });

  app.get('/api/trips/:tripId/hotel-rsvp-invites', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const invites = await storage.getPendingHotelRsvpsForUser(tripId, userId);
      res.json(invites);
    } catch (error) {
      console.error("Error fetching pending hotel RSVP invites:", error);
      res.status(500).json({ message: "Failed to fetch hotel RSVP invites" });
    }
  });

  app.get('/api/trips/:tripId/flight-rsvp-invites', isAuthenticated, async (req: any, res) => {
    try {
      const tripId = parseInt(req.params.tripId, 10);
      const userId = getRequestUserId(req);
      ...
      const invites = await storage.getPendingFlightRsvpsForUser(tripId, userId);
      res.json(invites);
    } catch (error) {
      console.error("Error fetching pending flight RSVP invites:", error);
      res.status(500).json({ message: "Failed to fetch flight RSVP invites" });
    }
  });
