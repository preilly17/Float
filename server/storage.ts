...
import {
  ...
  type FlightRsvp,
  type FlightRsvpWithUser,
  type HotelRsvp,
  type HotelRsvpWithUser,
  type RestaurantRsvp,
  type RestaurantRsvpWithUser,
} from "@shared/schema";
...
const mapActivity = (row: ActivityRow): Activity => ({
  ...
  bookingUrl: row.data && typeof row.data.bookingUrl === 'string' ? row.data.bookingUrl : null,
  createdAt: row.created_at,
  updatedAt: row.updated_at,
});
...
export class DatabaseStorage implements IStorage {
  ...
  async transferTripOwnership(
    ...
    if (trip.created_by !== currentOwnerId) {
      throw new Error("Only the trip creator can transfer ownership");
    }

    const { rows: memberCheck } = await query<{ user_id: string }>(
      `SELECT user_id FROM trip_members WHERE trip_calendar_id = $1 AND user_id = $2`,
      [tripId, newOwnerId]
    );

    if (memberCheck.length === 0) {
      throw new Error("New owner must be a member of the trip");
    }

    await query(
      `UPDATE trip_calendars SET created_by = $1, updated_at = NOW() WHERE id = $2`,
      [newOwnerId, tripId]
    );
  ...
  async createActivityWithInvites(
    ...
    try {
      ...
      if (activity.bookingUrl && typeof activity.bookingUrl === 'string' && activity.bookingUrl.trim()) {
        dataPayload.bookingUrl = activity.bookingUrl.trim();
      }
      ...
      const { rows } = await client.query<ActivityRow>(
        ...
        [
          ...
          typeValue,
          votingDeadlineValue,
          Object.keys(dataPayload).length > 0 ? JSON.stringify(dataPayload) : null,
        ],
      );
  ...
  async addPackingItem(
    ...
    const { rows } = await query<PackingItemRow>(
      ...
      RETURNING
        ...
        category,
        item_type,
        is_packed AS is_checked,
        assigned_to AS assigned_user_id,
        created_at
      `,
      [
        item.tripId,
  ...
  async createGroceryItem(
    ...
    const { rows } = await query<GroceryItemRow>(
      ...
      RETURNING
        ...
        NULL::text AS receipt_line_item,
        created_at,
        updated_at
      `,
      [
...
${selectUserColumns("added_by_user", "added_by_")}
      ...
      LEFT JOIN users added_by_user ON added_by_user.id = gi.created_by
      WHERE gi.trip_id = $1
...
${selectUserColumns("participant_user", "participant_user_")}
  ...
  async markGroceryItemPurchased(
    itemId: number,
    _actualCost?: string | number | null,
    isPurchased = true,
  ): Promise<void> {
    const { rows } = await query<{ id: number }>(
      `
...
${selectUserColumns("participant_user", "participant_user_")}
  ...
  async createFlightRsvpsForTripMembers(
    flightId: number,
    tripId: number,
    ...
    if (selectedMemberIds && selectedMemberIds.length > 0) {
      memberIds = selectedMemberIds.filter(id => validTripMemberIds.has(id));
    } else {
  ...
  async createHotel(
    ...
    const { rows } = await query<HotelRow>(
      ...
      [
        ...
        toJsonOrNull(getValue("amenities")),
        toJsonOrNull(getValue("images")),
        toJsonOrNull(getValue("policies")),
        toJsonOrNull(getValue("contactInfo")),
  ...
  async createHotelRsvpsForTripMembers(
    hotelId: number,
    tripId: number,
    creatorUserId: string,
    selectedMemberIds?: string[]
  ...
  async respondToHotelRsvp(
    ...
    const { rows } = await query<{
      ...
      `UPDATE hotel_rsvps
       SET status = $1, responded_at = NOW(), updated_at = NOW()
       WHERE hotel_id = $2 AND user_id = $3
       RETURNING id, hotel_id, user_id, status, responded_at, created_at, updated_at`,
      [status, hotelId, userId]
    );
  ...
  async getHotelRsvps(hotelId: number): Promise<HotelRsvpWithUser[]> {
    const { rows } = await query<{
      id: number;
      hotel_id: number;
      user_id: string;
      ...
      `SELECT hr.id, hr.hotel_id, hr.user_id, hr.status, hr.responded_at, hr.created_at, hr.updated_at,
              u.email as user_email, u.username as user_username, u.first_name as user_first_name,
              u.last_name as user_last_name, u.profile_image_url as user_profile_image_url
       FROM hotel_rsvps hr
  ...
  async getPendingHotelRsvpsForUser(tripId: number, userId: string): Promise<Array<{
    rsvp: HotelRsvp;
    hotel: Hotel;
    inviterName: string;
  }>> {
    const { rows } = await query<{
      ...
      h_city: string | null;
      h_country: string | null;
      h_check_in_date: Date | null;
      h_check_out_date: Date | null;
      h_price_per_night: string | null;
      h_total_price: string | null;
      h_status: string | null;
      h_created_by: string;
      ...
      `SELECT 
        hr.id as rsvp_id, hr.hotel_id, hr.user_id, hr.status, hr.responded_at, 
        hr.created_at as rsvp_created_at, hr.updated_at as rsvp_updated_at,
        h.id as h_id, h.name as h_name, h.address as h_address, h.city as h_city,
        h.country as h_country, h.check_in_date as h_check_in_date, h.check_out_date as h_check_out_date,
        h.price_per_night as h_price_per_night, h.total_price as h_total_price,
        h.status as h_status, h.created_by as h_created_by,
    ...
    return rows.map(row => ({
      ...
      hotel: {
        ...
        checkInDate: row.h_check_in_date,
        checkOutDate: row.h_check_out_date,
        pricePerNight: row.h_price_per_night,
        totalPrice: row.h_total_price,
        status: row.h_status,
        createdBy: row.h_created_by,
      } as Hotel,
  ...
  async getPendingFlightRsvpsForUser(tripId: number, userId: string): Promise<Array<{
    rsvp: FlightRsvp;
    flight: Flight;
    inviterName: string;
  }>> {
  ...
  async getRestaurantById(restaurantId: number): Promise<{ id: number; tripId: number; userId: string; name: string } | null> {
    const { rows } = await query<{ id: number; trip_id: number; created_by: string; name: string }>(
      `SELECT id, trip_id, created_by, name FROM restaurants WHERE id = $1`,
      [restaurantId],
    );
  ...
  async createRestaurantRsvpsForTripMembers(
    restaurantId: number,
    tripId: number,
    creatorUserId: string,
    selectedMemberIds?: string[]
  ...
  async respondToRestaurantRsvp(
    restaurantId: number,
    userId: string,
    status: 'accepted' | 'declined'
  ): Promise<RestaurantRsvp> {
    const { rows } = await query<{
      id: number;
      ...
      `UPDATE restaurant_rsvps
       SET status = $1, responded_at = NOW(), updated_at = NOW()
       WHERE restaurant_id = $2 AND user_id = $3
       RETURNING id, restaurant_id, user_id, status, responded_at, created_at, updated_at`,
      [status, restaurantId, userId]
    );
  ...
  async getRestaurantRsvps(restaurantId: number): Promise<RestaurantRsvpWithUser[]> {
    const { rows } = await query<{
      id: number;
      restaurant_id: number;
      user_id: string;
      ...
      `SELECT rr.id, rr.restaurant_id, rr.user_id, rr.status, rr.responded_at, rr.created_at, rr.updated_at,
              u.email as user_email, u.username as user_username, u.first_name as user_first_name,
              u.last_name as user_last_name, u.profile_image_url as user_profile_image_url
       FROM restaurant_rsvps rr
  ...
  async updateHotelProposalStatus(
    ...
    if (status === 'scheduled' || status === 'confirmed') {
      const { rows: rankerRows } = await query<{ user_id: string }>(
        `SELECT DISTINCT user_id FROM hotel_rankings WHERE proposal_id = $1`,
        [proposalId]
      );
      const rankerIds = rankerRows.map(r => r.user_id);
      
      if (!rankerIds.includes(currentUserId)) {
        rankerIds.push(currentUserId);
      }

      if (!proposal.checkInDate || !proposal.checkOutDate) {
        throw new Error("Cannot convert proposal: check-in and check-out dates are required");
      }

      const locationParts = (proposal.location || '').split(',').map(s => s.trim());
      const address = locationParts[0] || proposal.location || 'Not specified';
      const city = locationParts[1] || locationParts[0] || 'Not specified';
      const country = locationParts[2] || locationParts[1] || 'Not specified';
      ...
      const hotelData = {
        ...
        hotelRating: proposal.rating || null,
      ...
      const hotel = await this.createHotel(hotelData, currentUserId);
  ...
  async confirmFlightProposal(
    ...
    if (!proposerId || !requesterId || proposerId !== requesterId) {
      throw new Error("Only the proposal creator can confirm this flight");
    }
    ...
    if (normalizedStatus === "canceled" || normalizedStatus === "cancelled") {
      throw new Error("Cannot confirm a canceled proposal");
    }
    ...
    if (existingLinks.length > 0 && existingLinks[0]?.scheduled_id) {
      flightId = existingLinks[0].scheduled_id;
    } else if (proposal.flightId) {
      ...
      const { rows: flightRows } = await query<{ id: number }>(
        ...
        [
          ...
          proposal.flightNumber,
          proposal.departureAirport,
          proposal.departureCode || null,
          proposal.departureTime,
          proposal.departureTerminal || null,
          proposal.arrivalAirport,
          proposal.arrivalCode || null,
          proposal.arrivalTime,
          proposal.arrivalTerminal || null,
          proposal.duration,
          proposal.stops ? String(proposal.stops) : null,
          proposal.aircraft,
          proposal.price ? parseFloat(proposal.price) : null,
          proposal.currency || 'USD',
          proposal.seatClass || null,
          'scheduled',
          null,
          proposal.bookingSource || proposal.platform || 'Proposal',
          proposal.purchaseUrl || proposal.bookingUrl || null,
        ]
      );

      flightId = flightRows[0]?.id;
      if (!flightId) {
        throw new Error("Failed to create scheduled flight from proposal");
      }
    ...
    await this.createFlightRsvpsForTripMembers(flightId, proposal.tripId, currentUserId);

    await query(
      `UPDATE flight_proposals SET status = 'confirmed', updated_at = NOW() WHERE id = $1`,
      [proposalId]
    );

    const updated = await this.getFlightProposalById(proposalId, currentUserId);
    if (!updated) {
      throw new Error("Failed to load confirmed flight proposal");
    }
    ...
    const { rows } = await query<RestaurantProposalRow>(
      ...
      [
        ...
        toDbJson({
          ...
          reservationUrl: proposal.reservationUrl ?? null,
          platform: proposal.platform ?? null,
          atmosphere: proposal.atmosphere ?? null,
          specialties: proposal.specialties ?? null,
          dietaryOptions: proposal.dietaryOptions ?? null,
          preferredMealTime: proposal.preferredMealTime ?? null,
          preferredDates: proposal.preferredDates ?? null,
  ...
  async updateRestaurantProposalStatus(
    ...
    if (status === "scheduled") {
      ...
      const reservationDate = preferredDates && preferredDates.length > 0 
        ? preferredDates[0] 
        : null;
      
      const mealTimeToReservationTime: Record<string, string> = {
        'breakfast': '09:00',
        'brunch': '11:00',
        'lunch': '12:30',
        'dinner': '19:00',
      ...
      const reservationTime = preferredMealTime 
        ? mealTimeToReservationTime[preferredMealTime] || '19:00'
        : '19:00';
      ...
      const restaurant = await this.createRestaurant({
        ...
        cuisineType: proposal.cuisineType || undefined,
        priceRange: proposal.priceRange || undefined,
        rating: proposal.rating ? String(proposal.rating) : undefined,
        phoneNumber: proposal.phoneNumber || undefined,
        website: proposal.website || undefined,
        reservationDate: reservationDate || undefined,
        reservationTime: reservationTime,
      }, currentUserId);

      const thumbsUpUserIds = (proposal.rankings || [])
        .filter((r: { ranking: number }) => r.ranking === 1)
        .map((r: { userId: string }) => r.userId);
      
      const allInvitedIds = [currentUserId, ...thumbsUpUserIds.filter((id: string) => id !== currentUserId)];
      
      for (const memberId of allInvitedIds) {
        await query(
          `INSERT INTO restaurant_rsvps (restaurant_id, user_id, status, responded_at)
           VALUES ($1, $2, 'accepted', NOW())
           ON CONFLICT (restaurant_id, user_id) DO NOTHING`,
    ...
    const updatedProposals = await this.fetchRestaurantProposals({
      proposalIds: [proposalId],
      currentUserId,
    });

    const detailed = updatedProposals[0];
    if (!detailed) {
  ...
  async updateRestaurantProposal(
    proposalId: number,
    data: {
    ...
    const existingData = typeof proposal.features === "object" && proposal.features !== null
      ? proposal.features
      : {};

    const updatedData = {
      ...existingData,
      phoneNumber: data.phoneNumber ?? (existingData as Record<string, unknown>).phoneNumber,
      website: data.website ?? (existingData as Record<string, unknown>).website,
      reservationUrl: data.reservationUrl ?? (existingData as Record<string, unknown>).reservationUrl,
      platform: data.platform ?? (existingData as Record<string, unknown>).platform,
      preferredMealTime: data.preferredMealTime ?? (existingData as Record<string, unknown>).preferredMealTime,
      preferredDates: data.preferredDates ?? (existingData as Record<string, unknown>).preferredDates,
    };
