...
import { generateCashAppUrl, generateVenmoUrl } from "@/lib/paymentUtils";
              ...
              <>
                {(() => {
                  const venmoUrl = generateVenmoUrl(
                    expense.paidBy,
                    yourShareAmount?.toFixed(2) ?? "0",
                    expense.description || "Trip expense"
                  );
                    ...
                    <>
                      {venmoUrl && (
                        <Button
                          variant="outline"
                          size="sm"
                          className="border-blue-500/30 bg-blue-500/10 text-blue-600 hover:bg-blue-500/20 dark:border-blue-400/30 dark:bg-blue-500/15 dark:text-blue-400 dark:hover:bg-blue-500/25"
                          onClick={(event) => {
                      ...
                      {cashAppUrl && (
                        <Button
                          variant="outline"
                          size="sm"
                          className="border-emerald-500/30 bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 dark:border-emerald-400/30 dark:bg-emerald-500/15 dark:text-emerald-400 dark:hover:bg-emerald-500/25"
                          onClick={(event) => {
                            event.stopPropagation();
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 text-sm text-muted-foreground">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {isCurrentUserDebtor ? (
              <>
                {(() => {
                  const shareAmount = currentUserTargetShare?.toFixed(2) ?? "0";
                  const venmoUrl = generateVenmoUrl(
                    activeExpense.paidBy,
                    shareAmount,
                    activeExpense.description || "Trip expense"
                  );
                  const cashAppUrl = generateCashAppUrl(
                    activeExpense.paidBy,
                    shareAmount
