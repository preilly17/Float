...
import { FormGrid, FormRow } from "@/components/search/FormGrid";
    ...
    return (
      <div ref={containerRef} id="hotel-search-panel" className="space-y-6">
        <SearchFormCard
          ...
          title="Find Lodging for This Trip"
          description={`Search lodging to float to your group or save to your trip plan.${
            trip?.destination && (trip?.startDate || trip?.endDate)
              ? ` ${trip.destination}${(trip.startDate || trip.endDate) ? " • " : ""}${trip.startDate ? format(new Date(trip.startDate), "MMM d") : "Start TBD"}${trip.endDate ? `–${format(new Date(trip.endDate), "MMM d")}` : ""}`
              : ""
          }`}
          resultCount={searchResults.length > 0 ? searchResults.length : undefined}
        >
            <form onSubmit={handleSearchSubmit} className="space-y-6">
              <FormGrid columns={4}>
                <FormRow className="md:col-span-2">
                  <FieldLabel htmlFor="hotel-destination">Destination</FieldLabel>
                  <SmartLocationSearch
                ...
                <FormRow>
                  <FieldLabel htmlFor="hotel-check-in">Check-in</FieldLabel>
                  <Input
                    id="hotel-check-in"
                    type="date"
                    value={checkInDate}
                ...
                <FormRow>
                  <FieldLabel htmlFor="hotel-check-out">Check-out</FieldLabel>
                  <Input
                    id="hotel-check-out"
                    type="date"
                    value={checkOutDate}
                ...
                <FormRow>
                  <FieldLabel htmlFor="hotel-adults">Adults</FieldLabel>
                  <Input
                    id="hotel-adults"
                    type="number"
                    min={1}
                ...
                <FormRow>
                  <FieldLabel htmlFor="hotel-children">Children</FieldLabel>
                  <Input
                    id="hotel-children"
                    type="number"
                    min={0}
                ...
                <FormRow>
                  <FieldLabel htmlFor="hotel-rooms">Rooms</FieldLabel>
                  <Input
                    id="hotel-rooms"
                    type="number"
                    min={1}
              ...
              <FormGrid columns={3}>
                <FormRow>
                  <FieldLabel htmlFor="hotel-price-range">Price range</FieldLabel>
                ...
                <FormRow>
                  <FieldLabel htmlFor="hotel-min-rating">Minimum rating</FieldLabel>
              ...
              <ExternalPlatformLinks>
                <ExternalLinkButton
                  ...
                  tooltip="Search Airbnb for vacation rentals"
                >
                  Search Airbnb
                </ExternalLinkButton>
                <ExternalLinkButton
                  onClick={() => handleExternalSearch("vrbo")}
                  tooltip="Search VRBO for vacation rentals"
                >
                  Search VRBO
                </ExternalLinkButton>
                <ExternalLinkButton
                  onClick={() => handleExternalSearch("expedia")}
                  tooltip="Search Expedia for hotels"
                >
                  Search Expedia
                </ExternalLinkButton>
                <SearchButton isLoading={isSearching} loadingText="Searching...">
                  Search Hotels
                </SearchButton>
              </ExternalPlatformLinks>
            </form>
