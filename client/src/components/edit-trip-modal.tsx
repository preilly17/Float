...
export function EditTripModal({ open, onOpenChange, trip }: EditTripModalProps) {
  ...
  const [affectedEvents, setAffectedEvents] = useState<{
    summary: { activities: number; flights: number; hotels: number; restaurants: number };
    details: {
      activities: Array<{ id: number; name: string; date: string }>;
      flights: Array<{ id: number; airline: string; flightNumber: string; date: string }>;
      hotels: Array<{ id: number; name: string; checkIn: string; checkOut: string }>;
      restaurants: Array<{ id: number; name: string; date: string }>;
    };
  } | null>(null);
  ...
  const checkDateImpact = async (startDate: string, endDate: string) => {
    try {
      const response = await apiRequest(`/api/trips/${trip.id}/check-date-impact`, {
        method: "POST",
        body: JSON.stringify({ startDate, endDate }),
      });
      return await response.json();
    } catch (error) {
      console.error("Failed to check date impact:", error);
      return null;
    }
  };

  const performSubmit = async (submitData: FormData, uploadResult: UploadResponse | null = null) => {
    try {
      setSaveState("saving");
      ...
      const description = reason.includes("Only the trip creator")
        ? reason
        : `We couldn't save this. Reason: ${reason}`;
  ...
  const onSubmit = async (_data: FormData) => {
    ...
    try {
      ...
      const originalStartDate = format(new Date(trip.startDate), "yyyy-MM-dd");
      const originalEndDate = format(new Date(trip.endDate), "yyyy-MM-dd");
      const newStartDate = currentValues.startDate || originalStartDate;
      const newEndDate = currentValues.endDate || originalEndDate;
      const datesChanged = newStartDate !== originalStartDate || newEndDate !== originalEndDate;
      ...
      if (datesChanged) {
        const impactResult = await checkDateImpact(newStartDate, newEndDate);
        if (impactResult?.hasAffectedEvents) {
          setAffectedEvents({
      ...
      await performSubmit(submitData, uploadResult);
    } catch (error) {
      if (uploadResult) {
        await deleteUploadedFile(uploadResult.storageKey);
      }
  ...
  return (
    ...
    <AlertDialog open={showDateWarning} onOpenChange={setShowDateWarning}>
      <AlertDialogContent className="max-w-md">
        <AlertDialogHeader>
          <AlertDialogTitle>Events Outside New Date Range</AlertDialogTitle>
          <AlertDialogDescription className="space-y-3">
            <p>
              Changing the trip dates will affect some scheduled events that fall outside the new date range:
            </p>
            {affectedEvents && (
              <div className="text-sm space-y-2 bg-amber-50 dark:bg-amber-900/20 p-3 rounded-md border border-amber-200 dark:border-amber-700">
                {affectedEvents.summary.activities > 0 && (
                  <p><strong>{affectedEvents.summary.activities}</strong> activities</p>
                )}
                {affectedEvents.summary.flights > 0 && (
                  <p><strong>{affectedEvents.summary.flights}</strong> flights</p>
                )}
                {affectedEvents.summary.hotels > 0 && (
                  <p><strong>{affectedEvents.summary.hotels}</strong> hotels</p>
                )}
                {affectedEvents.summary.restaurants > 0 && (
                  <p><strong>{affectedEvents.summary.restaurants}</strong> restaurants</p>
                )}
              </div>
            )}
            <p className="text-muted-foreground">
              These items will no longer appear on the trip calendar. Do you want to continue?
            </p>
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>Cancel</AlertDialogCancel>
          <AlertDialogAction onClick={handleConfirmDateChange}>
            Continue Anyway
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
