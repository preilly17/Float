...
const dashboardSteps: Step[] = [
  ...
  },
  {
    ...
    icon: Calendar,
    tip: 'Use "Add Activity" to float ideas the whole crew can weigh in on',
  },
  {
...
export default function HowItWorks() {
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200">
      ...
      <section className="bg-gradient-to-br from-white via-primary/10 to-emerald-50/80 dark:from-slate-900 dark:via-primary/5 dark:to-emerald-950/30">
        <div className="mx-auto max-w-5xl px-6 py-16">
          <div className="max-w-3xl space-y-4">
            ...
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white md:text-4xl">How Float works</h1>
      ...
      <main className="mx-auto max-w-5xl space-y-12 px-6 py-12">
        <Card className="border-none bg-white dark:bg-slate-800 shadow-xl ring-1 ring-primary/10 dark:ring-primary/20">
          <CardHeader className="pb-4">
            <CardTitle className="text-2xl text-slate-900 dark:text-white">Home dashboard essentials</CardTitle>
          ...
          <CardContent className="pt-4">
            <ol className="space-y-4">
              {dashboardSteps.map((step, index) => (
                <li
                  key={step.title}
                  className="group rounded-2xl border border-slate-200/80 dark:border-slate-700 bg-white dark:bg-slate-800/50 p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md"
                >
                  ...
                  {step.tip && (
                    <div className="mt-3 rounded-lg border border-sky-100 dark:border-sky-900 bg-sky-50 dark:bg-sky-950/50 px-4 py-3 text-sm text-sky-700 dark:text-sky-300">
                      💡 {step.tip}
                    </div>
                  )}
                </li>
        ...
        <Card className="border-none bg-white dark:bg-slate-800 shadow-xl ring-1 ring-emerald-100/60 dark:ring-emerald-900/40">
          <CardHeader className="pb-4">
            <CardTitle className="text-2xl text-slate-900 dark:text-white">Deep-dive into trip planning</CardTitle>
            <CardDescription className="text-base text-slate-600 dark:text-slate-300">
          ...
          <CardContent className="pt-4">
            <ol className="space-y-4">
              {tripSteps.map((step, index) => (
                <li
                  key={step.title}
                  className="group rounded-2xl border border-slate-200/80 dark:border-slate-700 bg-white dark:bg-slate-800/50 p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-300 dark:hover:border-emerald-700 hover:shadow-md"
                >
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                    ...
                    <Badge
                      variant="outline"
                      className="border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/50 text-xs font-medium uppercase tracking-wide text-emerald-700 dark:text-emerald-400"
                    >
