...
export default function MemberSchedule() {
  ...
  useEffect(() => {
    if (trip && trip.members.length > 0 && !selectedMemberId) {
      // Default to current user ("Me") if they're a member
      const currentUserMember = trip.members.find(member => member.userId === currentUser?.id);
      if (currentUserMember) {
        setSelectedMemberId(currentUserMember.userId);
      } else if (trip.members.length > 0) {
        // Fall back to first member if current user isn't a member
