...
export default function Activities() {
  ...
  // Set default date range from trip dates
  useEffect(() => {
    if (!trip || dateRange) {
      return;
    }
    ...
    if (startDate && !Number.isNaN(startDate.getTime())) {
      setDateRange({
        from: startDate,
        to: endDate && !Number.isNaN(endDate.getTime()) ? endDate : undefined,
      });
    }
  }, [trip, dateRange]);
          ...
          <Collapsible open={isSearchPanelOpen} onOpenChange={setIsSearchPanelOpen}>
            {isSearchPanelOpen && (
              <SearchFormCard
                ...
                subtitle={`Find things to do in ${trip?.destination || 'your destination'}`}
                icon={<MapPin className="h-5 w-5" />}
                onClose={handleClosePanel}
              >
                  <form className="space-y-6" onSubmit={handleSearchSubmit}>
                    <FormGrid columns={4}>
                      <FormRow className="md:col-span-2 lg:col-span-1">
                    ...
                    <FormGrid columns={4}>
                      ...
                      <FormRow className="flex items-center gap-3">
                        ...
                        />
                        <FieldLabel htmlFor="free-cancellation" className="mb-0 cursor-pointer">
                          Free cancellation only
                        </FieldLabel>
                      </FormRow>
                    </FormGrid>

                    <ExternalPlatformLinks label="Or book directly on:">
                      <ExternalLinkButton
                        onClick={() => window.open(`https://www.viator.com/searchResults/all?text=${encodeURIComponent(locationSearch || trip?.destination || '')}`, '_blank')}
                      >
                        Search Viator
                      </ExternalLinkButton>
                      <ExternalLinkButton
                        onClick={() => window.open(`https://www.getyourguide.com/s/?q=${encodeURIComponent(locationSearch || trip?.destination || '')}`, '_blank')}
                      >
                        Search GetYourGuide
                      </ExternalLinkButton>
                      <SearchButton
                  ...
                  <div className="border-t border-neutral-200 pt-6 mt-6">
                    {!hasSearched ? (
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center justify-between">
                          <p className="text-sm text-neutral-600">
                            Showing {Math.min(displayCount, (searchActivities ?? []).length)} of {(searchActivities ?? []).length} activities
                          </p>
                          <Button
                            type="button"
                            variant="ghost"
                              ...
                              <CardContent className="flex flex-col flex-1 pt-2">
                                ...
                                <div className="space-y-2 mt-4 pt-3 border-t border-gray-100">
                                  <Button
                                    ...
                                    className="w-full bg-primary hover:bg-primary/90 text-white text-xs lg:text-sm"
                                  >
