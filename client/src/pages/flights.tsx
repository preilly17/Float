...
import { Plane, Clock, Users, Edit, Trash2, Plus, Search, ArrowLeft, PlaneTakeoff, PlaneLanding, ExternalLink, Loader2, CalendarCheck, CalendarIcon } from "lucide-react";
...
const buildSkyscannerUrl = (params: {
  origin: string;
  destination: string;
  ...
  const { origin, destination, departureDate, returnDate, passengers = 1, cabinClass } = params;
  const outbound = formatSkyscannerDate(departureDate);
  const inbound = returnDate ? formatSkyscannerDate(returnDate) : "";
  ...
  const cabin = cabinMap[cabinClass ?? "ECONOMY"] ?? "economy";
  
  let url = `https://www.skyscanner.com/transport/flights/${origin.toLowerCase()}/${destination.toLowerCase()}/${outbound}`;
  if (inbound) {
    url += `/${inbound}`;
  }
  url += `/?adults=${passengers}&cabinclass=${cabin}`;
  
  return url;
};

const buildPointHoundUrl = (params: {
  origin: string;
  destination: string;
  departureDate: string;
  returnDate?: string;
  ...
  const baseUrl = "https://www.pointhound.com/search";
...
function FlightSearchPanel({
  ...
  // Auto-fetch airports when trip destination is prefilled (has coordinates but no airports loaded yet)
  const hasAutoFetchedArrivalAirportsRef = useRef<string | null>(null);
  
  // Helper to check if a string is a valid IATA airport code (3 uppercase letters)
  const isValidIataCode = (code: string | undefined | null): boolean => {
    if (!code || typeof code !== 'string') return false;
    return /^[A-Z]{3}$/.test(code.trim().toUpperCase());
  };
  
  useEffect(() => {
    // Use coordinates from searchFormData (set during prefill) for airport lookup
    const arrLat = searchFormData.arrivalLatitude;
    const arrLng = searchFormData.arrivalLongitude;
    const arrCity = searchFormData.arrivalCity;
    const currentArrival = searchFormData.arrival;

    // Create a unique key to track what we've already fetched for
    const fetchKey = arrCity && arrLat != null && arrLng != null 
      ? `${arrCity}|${arrLat}|${arrLng}` 
      : null;

    // Only auto-fetch if:
    // 1. Arrival city and coordinates are set (from prefill)
    // 2. No airports have been loaded yet
    // 3. Haven't already auto-fetched for this destination
    // 4. Not currently loading
    // 5. Current arrival is not a valid IATA code (meaning no airport selected yet)
    const hasValidIataArrival = isValidIataCode(currentArrival);
    ...
    if (
      ...
      fetchKey !== hasAutoFetchedArrivalAirportsRef.current &&
      !isLoadingArrivalAirports &&
      !hasValidIataArrival
    ) {
      hasAutoFetchedArrivalAirportsRef.current = fetchKey;
      setIsLoadingArrivalAirports(true);

      const tripCityName = (trip as any)?.cityName;
      const tripCountryName = (trip as any)?.countryName;

      const locationForLookup = {
        latitude: Number(arrLat),
        longitude: Number(arrLng),
        name: tripCityName || arrCity,
        cityName: tripCityName,
        countryName: tripCountryName,
      };

      fetchNearestAirportsForLocation(locationForLookup)
        .then((result) => {
          if (result.airports.length > 0) {
            setArrivalAirports(result.airports);
            const firstAirport = result.airports[0];
            setSelectedArrivalAirport(firstAirport.iata);
            setSearchFormData((prev) => ({
              ...prev,
              arrival: firstAirport.iata,
            }));
          }
        })
        .catch((error) => {
          console.error('Failed to auto-fetch arrival airports:', error);
        })
        .finally(() => {
          setIsLoadingArrivalAirports(false);
        });
    }
  }, [trip, searchFormData.arrivalCity, searchFormData.arrivalLatitude, searchFormData.arrivalLongitude, searchFormData.arrival, arrivalAirports.length, isLoadingArrivalAirports, setSearchFormData]);
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3 border-t pt-4">
                  <span className="text-sm font-medium text-muted-foreground">Or book directly on:</span>
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => {
                      const depLocationName = departureLocation?.name || departureLocation?.displayName || '';
                      const arrLocationName = arrivalLocation?.name || arrivalLocation?.displayName || '';
                      const origin = selectedDepartureAirport || searchFormData.departureCity || depLocationName;
                      const destination = selectedArrivalAirport || searchFormData.arrivalCity || arrLocationName;
                      console.log('Skyscanner click:', { origin, destination, selectedDepartureAirport, selectedArrivalAirport, departureCity: searchFormData.departureCity, arrivalCity: searchFormData.arrivalCity, depLocationName, arrLocationName });
                      if (!origin || !destination || !searchFormData.departureDate) {
                        toast({
                          title: "Missing Information",
                          description: "Please enter departure, arrival, and departure date to search",
                          variant: "destructive",
                        });
                        return;
                      }
                      if (searchFormData.tripType === "roundtrip" && !searchFormData.returnDate) {
                        toast({
                      ...
                      const passengers = parseInt(searchFormData.passengers) || 1;
                      const url = buildSkyscannerUrl({
                        origin,
                        destination,
                        departureDate: searchFormData.departureDate,
                        returnDate: searchFormData.tripType === "roundtrip" ? searchFormData.returnDate : undefined,
                        passengers,
                      ...
                      if (!newWindow) {
                        toast({
                          title: "Popup Blocked",
                          description: "Please allow popups and try again, or copy this URL: " + url,
                          variant: "destructive",
                        });
                      }
                    }}
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      const url = buildPointHoundUrl({
                        origin,
                        destination,
                        departureDate: searchFormData.departureDate,
                        returnDate: searchFormData.tripType === "roundtrip" ? searchFormData.returnDate : undefined,
                      });
                      console.log('Point Hound URL:', url);
                      const newWindow = window.open(url, "_blank");
                      if (!newWindow) {
                        toast({
                          title: "Popup Blocked",
                              ...
                              <AccordionTrigger className="flex w-full items-center justify-between gap-4 px-4 py-4 text-left hover:bg-muted/50 [&[data-state=open]]:rounded-t-lg">
                                <div className="flex flex-col gap-2 text-left">
                                  <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <StatusBadge status="available" type="flight" />
                                  </div>
...
export default function FlightsPage() {
  ...
  // Member selection helpers for RSVP invites
  ...
  const handleToggleMember = useCallback((memberId: string, checked: boolean) => {
    setManualFlightForm((prev) => {
      const currentIds = prev.selectedMemberIds;
      if (checked && !currentIds.includes(memberId)) {
        return { ...prev, selectedMemberIds: [...currentIds, memberId] };
      } else if (!checked && currentIds.includes(memberId)) {
        return { ...prev, selectedMemberIds: currentIds.filter((id) => id !== memberId) };
      }
      return prev;
    });
  }, []);
  ...
  const handleSelectAllMembers = useCallback(() => {
    setManualFlightForm((prev) => ({
      ...prev,
      selectedMemberIds: memberOptions.map((m) => m.id),
    }));
  }, [memberOptions]);
  ...
  const handleClearMembers = useCallback(() => {
    setManualFlightForm((prev) => ({
      ...prev,
      selectedMemberIds: currentUserId ? [currentUserId] : [],
    }));
  }, [currentUserId]);

  // Initialize selected members when opening form in SAVE mode
  useEffect(() => {
    if (flightMode === 'SAVE' && manualFlightForm.selectedMemberIds.length === 0 && currentUserId) {
      setManualFlightForm((prev) => ({
        ...prev,
        selectedMemberIds: [currentUserId],
      }));
    }
  }, [flightMode, manualFlightForm.selectedMemberIds.length, currentUserId]);
  ...
  useEffect(() => {
    if (user && trip && !hasPrefilledSearch) {
      ...
      // Prefill arrival with trip destination (include coordinates for airport lookup)
      if ((trip as any).destination) {
        if (!searchFormData.arrivalCity) {
          newSearchData.arrivalCity = (trip as any).destination;
          // Include trip coordinates so FlightSearchPanel can auto-fetch airports
          const tripLat = (trip as any).latitude;
          const tripLng = (trip as any).longitude;
          newSearchData.arrivalLatitude = tripLat != null ? Number(tripLat) : null;
          newSearchData.arrivalLongitude = tripLng != null ? Number(tripLng) : null;
          hasChanges = true;
        }
      ...
      // Always prefill dates with trip dates (override any defaults)
      if ((trip as any).startDate) {
        const tripStartDate = format(new Date((trip as any).startDate), 'yyyy-MM-dd');
        newSearchData.departureDate = tripStartDate;
        hasChanges = true;
      }

      if ((trip as any).endDate) {
        const tripEndDate = format(new Date((trip as any).endDate), 'yyyy-MM-dd');
        newSearchData.returnDate = tripEndDate;
        hasChanges = true;
      }
  ...
  const handleFlightSubmit = () => {
    ...
    const insertPayload: InsertFlight = {
      ...
      currency: 'USD',
      flightType: null,
      status: editingFlight?.status || (flightMode === 'SAVE' ? 'scheduled' : 'proposed'),
      bookingReference: null,
    ...
    const manualPayload: ManualFlightPayload = {
      ...
      priceCents: null,
      direction: 'OUTBOUND',
      selectedMemberIds: flightMode === 'SAVE' ? manualFlightForm.selectedMemberIds : undefined,
    };
        ...
        <section className="space-y-4">
          ...
          {sortedFlights.length === 0 ? (
            <EmptyState
              ...
              description="Search for flights above or add your already-booked flights manually to keep your group coordinated."
              actionLabel="Add Flight Manually"
              onAction={() => setIsAddFlightOpen(true)}
            />
          ) : (
            <Accordion
              type="single"
              ...
              return (
                ...
                >
                  <AccordionTrigger className="flex w-full items-center gap-4 px-4 py-4 text-left hover:bg-muted/50 [&[data-state=open]]:rounded-t-lg">
                    <div className="flex w-full flex-wrap items-center justify-between gap-4">
                      <div className="flex flex-col gap-1 text-left">
                        ...
                        {flight.user && (
                          <AttributionLabel
                            name={flight.user.firstName || flight.user.username || "Unknown"}
                            isCurrentUser={flight.user.id === currentUserId}
                            variant="booked"
                          />
                        )}
                      </div>
                  ...
                  <AccordionContent className="px-4 pb-4">
                    ...
                    {currentUserId && (
                      <FlightRsvpSection flightId={flight.id} currentUserId={currentUserId} />
                    )}
    ...
    >
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-4">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {(() => {
                      const depDate = manualFlightForm.departure ? new Date(manualFlightForm.departure) : null;
                ...
                <PopoverContent className="w-auto p-3" align="start">
                  <Calendar
                    mode="range"
                    defaultMonth={manualFlightForm.departure ? new Date(manualFlightForm.departure) : undefined}
                    selected={(() => {
                      const depDate = manualFlightForm.departure ? new Date(manualFlightForm.departure) : undefined;
                      const arrDate = manualFlightForm.arrival ? new Date(manualFlightForm.arrival) : undefined;
                      if (depDate && !isNaN(depDate.getTime())) {
                        return { from: depDate, to: arrDate && !isNaN(arrDate.getTime()) ? arrDate : depDate } as DateRange;
                      }
                      return undefined;
                    })()}
                    onSelect={(range) => {
                      if (range?.from) {
                        ...
                        setManualFlightForm((prev) => ({ ...prev, departure: newDepDate }));
          ...
          {flightMode === 'SAVE' && !editingFlight && (
            <MemberSelector
              members={memberOptions}
              selectedMemberIds={manualFlightForm.selectedMemberIds}
            ...
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-3">
              ...
              <div>
                ...
                <Input
                  id="voting-deadline"
                  type="datetime-local"
                  value={manualFlightForm.votingDeadline}
