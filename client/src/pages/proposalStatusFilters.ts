...
const CONVERTED_STATUSES = new Set(["scheduled", "confirmed", "converted", "booked"]);
...
export const isConvertedStatus = (status?: string | null): boolean =>
  CONVERTED_STATUSES.has(normalizeProposalStatus(status));

export const filterActiveProposals = <T extends { status?: string | null }>(items: T[]): T[] =>
  items.filter((item) => !isCanceledStatus(item.status) && !isConvertedStatus(item.status));
