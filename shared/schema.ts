...
export interface FlightRsvpWithUser extends FlightRsvp {
  user: User;
}

export interface FlightWithRsvps extends Flight {
  rsvps: FlightRsvpWithUser[];
  currentUserRsvp?: FlightRsvp;
  createdByUser?: User;
}

export interface HotelRsvp {
  id: number;
  hotelId: number;
  userId: string;
  status: RsvpStatus;
...
export interface HotelRsvpWithUser extends HotelRsvp {
  user: User;
}

export interface HotelWithRsvps extends Hotel {
  rsvps: HotelRsvpWithUser[];
  currentUserRsvp?: HotelRsvp;
  createdByUser?: User;
}
...
export const insertHotelProposalSchema = z.object({
  ...
  country: z.string().min(1, "Country is required"),
  checkInDate: z.union([z.date(), z.string()]).refine((val) => val !== null && val !== undefined, {
    message: "Check-in date is required",
  }),
  checkOutDate: z.union([z.date(), z.string()]).refine((val) => val !== null && val !== undefined, {
    message: "Check-out date is required",
  }),
  price: z.string().optional().default(""),
  pricePerNight: z.union([z.string(), z.number()]).nullable().optional().transform((val) => {
    if (val === null || val === undefined || val === "") return null;
    return String(val);
  }),
  guestCount: z.union([z.number(), z.string()]).transform((val) => {
    if (typeof val === "string") return parseInt(val) || 1;
    return val;
  }),
  roomCount: z.union([z.number(), z.string()]).nullable().optional().transform((val) => {
    if (val === null || val === undefined || val === "") return null;
  ...
  bookingReference: z.string().optional().default(""),
  bookingSource: z.string().optional().default(""),
  bookingPlatform: z.string().optional().default(""),
  purchaseUrl: z.string().optional().default(""),
  hotelChain: z.string().optional().default(""),
...
export const insertRestaurantRankingSchema = z.object({
  proposalId: z.number(),
  ranking: z.number().min(-1), // -1 = Not for me, 0 = No vote/toggle off, 1 = I'm in
  notes: z.string().nullable().optional(),
});
...
export interface RestaurantRsvp {
  id: number;
  restaurantId: number;
  userId: string;
  status: RsvpStatus;
...
export interface RestaurantRsvpWithUser extends RestaurantRsvp {
  user: User;
}

export interface RestaurantWithRsvps extends Restaurant {
  rsvps: RestaurantRsvpWithUser[];
  currentUserRsvp?: RestaurantRsvp;
  createdByUser?: User;
}
